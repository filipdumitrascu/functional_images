...
-}
newtype RegionAST
    = R (RegionShape RegionAST) -- RegionAST este un RegionShape accesat prin costruct de date R(newtype)
    deriving (Eq)
...
--  intercalate :: [a] -> [[a]] -> [a]; inserts "," between the strings and concats
...
indent :: Int -> String
indent level = replicate (level * 2) ' ' -- indent has two spaces
-- replicate :: Int -> a -> [a]

showIndented :: Int -> RegionAST -> [String]
showIndented level (R shape) = case shape of
    ...
    Complement region -> [indent level ++ "~"] ++ showIndented (level + 1) region
    Union region1 region2 -> [indent level ++ "+"] ++ showIndented (level + 1) region1 ++
                             showIndented (level + 1) region2
    Intersection region1 region2 -> [indent level ++ "*"] ++ showIndented (level + 1) region1 ++
                                    showIndented (level + 1) region2
    Transform transformation region -> [indent level ++ show transformation] ++
                                       showIndented (level + 1) region

instance Show RegionAST where
    show region = intercalate "\n" (showIndented 0 region)
...
instance Functor TransformationShape where
    ...
    fmap f (Translation tx ty) = Translation tx ty -- nu contin typeu a 
    fmap f (Scaling factor) = Scaling factor
    fmap f (Combine xs) = Combine (map f xs) -- contin, se aplica f
...
instance Functor RegionShape where
    ...
    fmap f (FromPoints points) = FromPoints points  -- nu contin a
    fmap f (Rectangle width height) = Rectangle width height
    fmap f (Circle radius) = Circle radius
    fmap f (Complement region) = Complement (f region) -- contin a 
...
-- fmap on AST to decompose it as a and f for the result
-- (Combiner combines the a inside the Shape in an a)
...
{-
    *** TODO ***
    ...
    Răspuns: calculeaza doar acea componenta
...
basicEntityCount = foldRegionAST combiner
  where
    ...
    combiner (Complement inner) = inner
    combiner (Union r1 r2) = combineCounts r1 r2
    combiner (Intersection r1 r2) = combineCounts r1 r2
    combiner (Transform trans inner) = transformCount trans inner

    transformCount :: TransformationAST -> (Int, Int) -> (Int, Int)
    transformCount (T (Translation _ _)) (r, t) = (r, t + 1)
    transformCount (T (Scaling _)) (r, t) = (r, t + 1)
    transformCount (T (Combine trnsfrms)) (r, t) = foldr combineCounts (r, t) (map countTransform trnsfrms)

    combineCounts :: (Int, Int) -> (Int, Int) -> (Int, Int)
    combineCounts (r1, t1) (r2, t2) = (r1 + r2, t1 + t2)

    -- counts the number of regions and transf in a region
    countTransform :: TransformationAST -> (Int, Int)
    countTransform trans = transformCount trans (0, 0)

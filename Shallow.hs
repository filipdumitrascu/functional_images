...
fromPoints = flip elem  -- [x1, x2, .. xn] ==> f (pentru inside)
...
rectangle width height = \(x, y) -> abs x <= width * 0.5 && abs y <= height * 0.5
-- coordonata este fata de centru ==> lungime / 2 
...
-}
circle :: Float -> Region
circle radius = \point -> dist (0, 0) point <= radius
    where dist (xA, yA) (xB, yB) = sqrt ((xB - xA) ^ 2 + (yB - yA) ^ 2)
...
plot width height region = intercalate "\n" [line y | y <- reverse [(-height)..height]]
  where
    line y = [plotChar x y | x <- [-width..width]]
    plotChar x y = if inside (fromIntegral x, fromIntegral y) region then '*' else '.'
...
complement = promoteUnary not  -- g ==> f(g())

union :: Region -> Region -> Region
union = promoteBinary (||) -- g, h ==> f(g(), h())

intersection :: Region -> Region -> Region
intersection = promoteBinary (&&) -- g, h ==> f(g(), h())
...
applyTransformation = flip (.) -- f, g ==> g(f()); I.transf II.reg
...
combineTransformations = foldr (flip (.)) id -- f, g, h  ==> f(g(h()))
...
{-
    *** TODO ***
    ...
    Răspuns: se construiesc doar cercurile pana la coordonatele punctului.
-}
circles :: Int -> Region
circles n
    | n <= 0    = const False
    | otherwise = {-trace ("circle: " ++ show p) $-} union (circle 2)
...
{-
    *** TODO ***
    ...
    Răspuns: pare ca se ruleaza la infinit.
-}
infiniteCircles :: Region
infiniteCircles = {-trace ("circle " ) $-} union (circle 2)
                                            (applyTransformation (translation 6 0)
                                                                 infiniteCircles)
...
bfs start expand = helper [(start, 0)] S.empty  -- [start(root), level], expand(neighbours)
    where
        helper [] _ = [] -- queue empty => done
        helper ((state, distance) : queue) visited -- for this pair
            | S.member state visited = helper queue visited -- already visited, next state
            | otherwise = (state, distance) : helper newQueue newVisited -- adds the state in the answer
                where
                    newQueue = queue ++ newStates -- adds the neighbours to the queue
                    newVisited = S.insert state visited -- consider the state visited
                    newStates = [(neighbour, distance + 1) | neighbour <- neighbours]
...
regionAvoidingBfs start region = bfs start expand
  where
    expand :: Point -> [Point] -- returns the neighbours list that are not in the region
    expand (x, y) = filter (\p -> not (inside p region)) [(x, y + 1), (x + 1, y), (x, y - 1), (x - 1, y)]

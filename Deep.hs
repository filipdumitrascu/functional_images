...
fuseTransformations = foldr fuse []
  where
    fuse (Translation x1 y1) ((Translation x2 y2) : rest) = Translation (x1 + x2) (y1 + y2) : rest
    fuse (Scaling s1) ((Scaling s2) : rest) = Scaling (s1 * s2) : rest
    fuse t acc = t : acc
...
-- remove the transformations applied by region
findInitialRegion :: RegionAST -> RegionAST
findInitialRegion region = case region of
    Transform transformation innerRegion -> findInitialRegion innerRegion
    Complement innerRegion -> Complement (findInitialRegion innerRegion)
    Union _ region2 -> findInitialRegion region2
    Intersection _ region2 -> findInitialRegion region2
    otherwise -> region

-- collects in a list all the transformations applied on a region 
collectAllTransformations :: RegionAST -> [TransformationAST]
collectAllTransformations region = case region of
    Transform transformation innerRegion -> decomposeTransformation transformation ++
                                            collectAllTransformations innerRegion
    Complement innerRegion -> collectAllTransformations innerRegion
    Union region1 region2 -> collectAllTransformations region1 ++ collectAllTransformations region2
    Intersection region1 region2 -> collectAllTransformations region1 ++ collectAllTransformations region2
    otherwise -> []

optimizeTransformations :: RegionAST -> RegionAST
optimizeTransformations region = case region of
    Transform _ _ ->
        applyTransformation composed initialRegion
    Complement _ ->
        applyTransformation composed initialRegion
    ...
    otherwise -> region
    where
        initialRegion = findInitialRegion region
        composed = combineTransformations (fuseTransformations (collectAllTransformations region))
